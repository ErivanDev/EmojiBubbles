...
window.addEventListener("mousemove", function (event) {
    if (mouse)
        ang = Math.atan2(event.offsetY - y, event.offsetX - x);
});
...
window.addEventListener("mousedown", function (event) {
    if (mouse == true && Math.dist(event.offsetX, event.offsetY, x, y) < 50) {
        troca = tipo;
        tipo = tipo2;
        tipo2 = troca;
    }
    else if (mouse == true && lancamento == true) {
        bola[bola.length] = new ball(cos(ang), sin(ang), tipo);
        tipo = tipo2;
        tipo2 = int(random(5));
        lancamento = false;
        ...
        mouse = true;
        document.getElementById("mouse").innerHTML = "Mouse Ativado";
    }
});
